...
**Ajuste de layout a 2 filas (2026-07-13, mismo día):** se cambió a exactamente 2 filas — fila 1: encabezado con título/descripción/instrucciones (ancho 4) + panel de actuador (ancho 2, switch y badge apilados); fila 2: Hombro/Codo/Muñeca (ancho 2 cada uno). Los anchos de cada grupo suman 6 por fila para que Node-RED los empaquete en la misma línea en vez de apilarlos verticalmente.

**Incidente detectado y resuelto:** entre el primer rediseño y este ajuste, el flujo en vivo volvió a la versión anterior (19 nodos, sin el rediseño). La causa fue una condición de carrera: el editor de Node-RED (`http://localhost:1880/`, distinto del dashboard `http://localhost:1880/ui`) mantiene su propia copia en memoria del flujo una vez que se carga la página, y si estaba abierto desde antes de un despliegue hecho por la API, un siguiente "Deploy" desde ese editor sobreescribe el cambio de la API con la versión vieja que tenía cacheada. No se perdió ningún trabajo manual — se verificó nodo por nodo que la versión que reapareció era exactamente la anterior generada por mí. **Mientras se siga iterando el diseño por la API, conviene no tener abierta la pestaña del editor (`/`, sin `/ui`) — o refrescarla antes de tocar nada ahí — para no volver a pisar el despliegue.**

**Corrección de layout (2026-07-13, mismo día):** el intento anterior (5 grupos separados con anchos que debían sumar 6 por fila) no funcionó — Node-RED no garantiza cómo se acomodan varios *grupos* entre sí (depende del ancho en píxeles real disponible en el navegador, no de una regla fija de "6 unidades por fila"), y además `site.sizes.cx/cy` estaba mal configurado en 6px (casi invisible) en vez de 48px. Resultado: todo se veía en una sola fila apretada con mucho espacio vacío.
